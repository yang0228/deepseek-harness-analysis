...
| UI 表面 | 同一类 hook 事件在终端、IDE 扩展、桌面应用和 Claude Code on the web 中触发；云端会话不读取本地用户 settings file。 | [Hooks reference](https://code.claude.com/docs/en/hooks)（Anthropic，访问 2026-09-08） | DSH 的 `web`、`headless` 和 `sdk` 等 CLI 入口由 Profile 组合，Desktop 另有独立 Host，见[架构概览](../02-architecture.md)与[可复现的入门路径](../05-getting-started.md)。 | 未见官方文档说明：指定页面没有对这些客户端的全部交互与会话能力作统一目录。 | 本行只比较公开入口，不把同一 hook 事件集解释为客户端完全等价。 |
